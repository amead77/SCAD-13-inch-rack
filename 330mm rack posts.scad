...
module post(
    ...
    union() {
        ...
        if (doublewide == 0) {
            if ((slide_side == 1) || (slide_side == 3)) {
                post_slider_left(post_width, u_height, post_slide_width, post_slide_cutout, hole_offset_z, hole_spacing);
            }
            if ((slide_side == 2) || (slide_side == 3)) {
                post_slider_right(post_width, u_height, post_slide_width, post_slide_cutout, hole_offset_z, hole_spacing);
            }
        } else {
            // doublewide
            if ((slide_side == 1) || (slide_side == 3)) {
...
module post_slider_left(
    ...
    translate([post_width, 0, 0]) {
        post_slides(post_width, u_height, post_slide_width, post_slide_cutout, hole_offset_z, hole_spacing);
    }
}
...
module rail_1u_holes_segment(
    ...
    difference() {
        post(slide_side, doublewide, post_width, u_height, post_slide_width, post_slide_cutout, hole_offset_z, hole_spacing);
        holes(holes = 3, post_width = post_width, hole_d = hole_d, hole_offset_z = hole_offset_z, hole_spacing = hole_spacing);
        nut_holes(holes = 3, post_width = post_width, nut_thickness = nut_thickness, nut_diameter_point = nut_diameter_point, hole_offset_z = hole_offset_z, hole_spacing = hole_spacing);
        if (doublewide == 1) {
            translate([post_width, 0, 0]) {
                holes(holes = 3, post_width = post_width, hole_d = hole_d, hole_offset_z = hole_offset_z, hole_spacing = hole_spacing);
...
module base_joiner_block(
    ...
    translate([0, 0, -(footer_height * 2)]) {
        difference() {
            cube([post_width, post_width, footer_height]);
            base_joiner_hole_and_nut(post_width, hole_offset_z, hole_d, nut_thickness, nut_diameter_point);
        }
    }
    if (doublewide == 1) {
        translate([post_width, 0, -(footer_height * 2)]) {
...
// doublewide: 0=single, 1=double. bottom: 1=base joiner, 0=top joiner.
// supports: minimum 2 (front + rear); >2 adds equally-spaced intermediate support blocks.
// beam_thickness: override beam thickness.
module base_joiner_core(
    doublewide = 0,
    bottom = 1,
    supports = 2,
    ...
    _beam_thickness = beam_thickness;
    union() {
        base_joiner_block(doublewide, post_width, footer_height, hole_offset_z, hole_d, nut_thickness, nut_diameter_point);
...
// beam_thickness: override beam thickness.
// e.g. base_joiner(doublewide=0);
// e.g. base_joiner(doublewide=0, bottom=0);
// e.g. base_joiner(doublewide=0, supports=4);
// e.g. base_joiner(doublewide=0, beam_thickness=8);
module base_joiner(
    doublewide = 0,
    bottom = 1,
    supports = 2,
    ...
    if (bottom == 1) {
        base_joiner_core(
            ...
            nut_thickness,
            nut_diameter_point,
            post_cone_base_diameter,
            post_cone_top_diameter,
            post_cone_height,
            post_top_cone_clearance
        );
    } else {
...
// doublewide: 0=single, 1=double.
// post_height: number of U units.
// v_post_cones: 1=add alignment cones for stacking.
// include_footer/include_header: 1=include attachment block.
...
module rail_1u_holes(
    ...
    difference() {
        union() {
            ...
            if (include_footer == 1) {
                footer_body(doublewide, post_width, footer_height);
            }
            if (include_header == 1) {
                header_body(post_height, doublewide, post_width, u_height, header_height);
            }

            if (v_post_cones == 1) {
                // top cones for joining rails
                if (include_header == 1) {
                    translate([post_width / 2, post_width / 2, u_height * post_height + header_height]) {
                        cylinder(h = post_cone_height, r1 = (post_cone_base_diameter / 2) - post_top_cone_clearance, r2 = (post_cone_top_diameter / 2) - post_top_cone_clearance, center = false, $fn = 32);
                    }
        ...
        if (include_header == 1) {
            header_hole_and_nut(post_height, post_width, u_height, hole_offset_z, hole_d, nut_thickness, nut_diameter_point);
            if (doublewide == 1) {
                translate([post_width, 0, 0]) {
                    header_hole_and_nut(post_height, post_width, u_height, hole_offset_z, hole_d, nut_thickness, nut_diameter_point);
                }
        ...
        if (include_footer == 1) {
            footer_hole_and_nut(post_width, hole_offset_z, hole_d, nut_thickness, nut_diameter_point);
            if (doublewide == 1) {
                translate([post_width, 0, 0]) {
                    footer_hole_and_nut(post_width, hole_offset_z, hole_d, nut_thickness, nut_diameter_point);
                }
